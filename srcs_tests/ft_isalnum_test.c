...
void ft_isalnum_test(void)
{
    ...
    for (int c = 'A'; c <= 'Z'; c++)
    {
        ...
        if (result != expected)
        {
            printf("%s[FAIL]%s ft_isalnum('%c') = %d, expected %d\n", 
    ...
    {
        total_tests++;
        int expected = isalnum(special[i]) ? 1 : 0;
        int result = ft_isalnum(special[i]);
        if (result != expected)
        {
            printf("%s[FAIL]%s ft_isalnum('%c') = %d, expected %d\n", 
    ...
    for (int c = 128; c < 256; c++)
    {
        ...
        if (result != expected)
        {
            printf("%s[FAIL]%s ft_isalnum(%d) = %d, expected %d\n", 
                RED, RESET, c, result, expected);
            failed_tests++;
        }
        else
