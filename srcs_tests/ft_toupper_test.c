...
void ft_toupper_test(void)
{
    ...
    {
        total_tests++;
        int expected = toupper(c);  // Use standard toupper for expected result
        int result = ft_toupper(c);
        if (result != expected)
        {
            printf("%s[FAIL]%s ft_toupper('%c') = '%c', expected '%c'\n", 
    ...
    {
        total_tests++;
        int expected = c;  // ft_toupper should return the same character
        int result = ft_toupper(c);
        if (result != expected)
        {
            printf("%s[FAIL]%s ft_toupper('%c') = '%c', expected '%c'\n", 
    ...
    while (non_alpha[i])
    {
        ...
        int result = ft_toupper(non_alpha[i]);
        if (result != expected)
        {
            printf("%s[FAIL]%s ft_toupper('%c') = '%c', expected '%c'\n", 
    ...
    while (j < sizeof(boundary_cases) / sizeof(boundary_cases[0]))
    {
        ...
        int expected;
        if (c >= 'a' && c <= 'z')
            expected = c - 32; // Convert to uppercase
        else if (c >= 'A' && c <= 'Z')
            expected = c;      // Already uppercase
        else if (c >= 0 && c <= 127)
        ...
        if (result != expected)
        {
            printf("%s[FAIL]%s ft_toupper(%d) = '%c', expected '%c'\n", 
                RED, RESET, c, result, expected);
            failed_tests++;
        }
        else
